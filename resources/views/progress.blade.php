...
@section('content')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', async () => {
            ...
            const lineName = @json(session('lineName'));
            const chatHistory = @json(session('chatHistory'));
            const partnerName = @json(session('partnerName'));

            const apiKeys = [
                @json(env('OPENAI_API_KEY1')),
                @json(env('OPENAI_API_KEY2')),
                @json(env('OPENAI_API_KEY3'))
            ];
            let apiKeyIndex = 0;
